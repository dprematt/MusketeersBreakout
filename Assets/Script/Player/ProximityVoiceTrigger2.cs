...
public class ProximityVoiceTrigger2 : VoiceComponent
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            ProximityVoiceTrigger2 trigger = other.GetComponent<ProximityVoiceTrigger2>();
            if (trigger != null)
            {
    ...
    private bool IsLocalCheck()
    {
        ...
        if (this.enabled)
        {
            this.Logger.LogInfo("Disabling ProximityVoiceTrigger2 as does not belong to local player, actor number {0}", this.TargetInterestGroup);
            this.enabled = false;
        }
        return false;
    }
